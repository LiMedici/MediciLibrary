...
public final class CrashUtil {
    ...
    static {
        try {
            PackageInfo pi = UIUtil.getApp().getPackageManager().getPackageInfo(UIUtil.getApp().getPackageName(), 0);
            if (pi != null) {
    ...
    public static void init(final String crashDir) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                && UIUtil.getApp().getExternalCacheDir() != null)
            defaultDir = UIUtil.getApp().getExternalCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        else {
            defaultDir = UIUtil.getApp().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        }
