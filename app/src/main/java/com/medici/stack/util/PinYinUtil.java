...
//            Locale newChina = new Locale("zh", "HANS", "CN");//这部分是我做的修复，解决HTC兼容性问题
            /*其中Locale.CHINA为“ch_CN”,然而locale数组中没有匹配到，而不同设备都有“ch”,将Local.CHINA改成Local.CHINESE搞定，它的字符是“ch”*/
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(/*Locale.CHINA*/Locale.CHINESE)/* || locale[i].equals(newChina)*/) {
    ...
    /**
     * The Chinese characters return to pinyin, and the letter is returned to the original, all lowercase
     * @param input
     * @return
     */
